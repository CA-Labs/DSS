...
dssApp.directive('reqInput', function() {
    return {
        ...
        link: function(scope, element) {
            ...
            attributes = scope.$eval("{" + scope.risk.destination.attributes + "}");
            placeholder = attributes.placeholder || "";
            type = attributes.type || "text";
            element.html('<label for="' + scope.risk.destination.name + '">' + scope.risk.destination.name + ' <span data-tooltip class="has-tip tip-top" title="' + scope.risk.destination.definition + '"><i class="fi-lightbulb"></i></span></label><input name="' + scope.risk.destination.linkName + '" type="' + type + '" class="large-12 columns query" placeholder="' + placeholder + '">');
...
dssApp.directive('reqRadio', function() {
    return {
        ...
        link: function(scope, element) {
            var attributes;
            attributes = scope.$eval("{" + scope.risk.destination.attributes + "}");
            element.html('<label>' + scope.risk.destination.name + '</label><label class="switch-light well" onclick=""><input type="checkbox"/><span><span>' + attributes[0] + '</span><span>' + attributes[1] + '</span></span><a class="btn btn-success"></a></label>');
...
dssApp.directive('reqSelect', function() {
    return {
        ...
        link: function(scope, element) {
            ...
            attributes = scope.$eval("{" + scope.risk.destination.options + "}");
            options = '<option value="none">-- select --</option>';
            angular.forEach(attributes, function(value, key) {
                options = options + '<option value="' + key + '">' + value + '</option>';
            });
            element.html('<label for="' + scope.risk.destination.name + '">' + scope.risk.destination.name + ' <span data-tooltip class="has-tip tip-top" title="' + scope.risk.destination.name + '"><i class="fi-lightbulb"></i></span></label><select name="' + scope.risk.destination.linkName + '" class="query form-control form">' + options + '</select>');
            return element.bind('change', function() {
                return scope.change();
            });
        }
