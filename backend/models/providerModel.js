...
var FoxxModel = require('org/arangodb/foxx').Model,
    ...
    ProviderModel = FoxxModel.extend({
        schema: {
            ...
            year_founding: joi.number().min(1800).max(Date.now.getFullYear()).optional(),
            website: joi.string().optional().regex("(http|ftp|https)://[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:/~+#-]*[\w@?^=%&amp;/~+#-])?"),
            logo_url: joi.string().optional().regex("(http|ftp|https)://[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:/~+#-]*[\w@?^=%&amp;/~+#-])?"),
